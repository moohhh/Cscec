...
function DemandeChauffeur() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // First Axios POST request to create a demand for a chauffeur
      const response1 = await axios.post('http://localhost:8000/api/demandechaffeur', {
      ...
      alert('Demande de chauffeur created successfully!');

      // Second Axios POST request to create a generic demand
      const response2 = await axios.post('http://localhost:8000/api/demande', {
        employe_id: employe_id,
        title: 'Demande de Chauffeur',
        description: 'Demande de Chauffeur le ' + date + ' a ' + time,
      });

      console.log(response2.data.message);
      alert('Generic demande created successfully!');
    } catch (error) {
